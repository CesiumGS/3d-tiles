...
The screenshot below shows the bounding volumes for the root tile for [Canary Wharf](http://cesiumjs.org/CanaryWharf/).  `box` is red, and enclosed the entire area of the tileset; `contents.box` is blue, and encloses just the four models in the root tile.
