...
A layer may also associate application-specific metadata by conforming to a `class` defined in [3DTILES_metadata](../../3DTILES_metadata/0.0.0). Metadata must be declared in conformance with the [Cesium 3D Metadata Specification](../../../specification/Metadata/0.0.0). The following diagram illustrates the relationship between these documents:
...
The [3DTILES_metadata extension](../../3DTILES_metadata/0.0.0) enables the declaration of metadata `class` objects, which define a list of properties a conforming instance would need to provide. A layer in 3DTILES_layers may assign to itself a class declared in 3DTILES_metadata to provide additional metadata. Layers must follow the [single instance syntax](../../../specification/Metadata/0.0.0#single-instance-shorthand) for assigning values to its `properties`. For example,
