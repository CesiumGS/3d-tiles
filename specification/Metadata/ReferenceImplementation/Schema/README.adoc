...
.Example
====
A "Species" enum defining types of trees. An "Unspecified" enum value is optional, but when provided as the `noData` value for a property (see: link:../../README.md#required-properties-no-data-values-and-default-values[3D Metadata ? No Data Values]) may be helpful to identify missing data.
