...
* The xref:Schema/README.adoc[Schema] is a JSON-based representation of xref:{url-specification-metadata}README.adoc#metadata-schemas[3D Metadata Schemas] that describe the structure and types of metadata
* The xref:PropertyTable/README.adoc[PropertyTable] is one form of a xref:{url-specification-metadata}README.adoc#metadata-binary-table-format[Binary Table Format]. It is a JSON-based description of how large amounts of metadata can be stored compactly in a binary form.
