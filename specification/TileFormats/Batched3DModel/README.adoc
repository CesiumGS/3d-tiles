...
The <<tileformats-batched3dmodel-binary-gltf,binary glTF>> shall start and end on an 8-byte boundary so that glTF's byte-alignment guarantees are met. This can be done by padding the Feature Table, or by padding the Batch Table if it is present. The actual binary glTF data does _not_ include possible trailing padding bytes. Clients must take into account the `length` from the https://registry.khronos.org/glTF/specs/2.0/glTF-2.0.html#binary-gltf-layout[binary glTF header], and use this length to determine the part of the tile data that actually represents the binary glTF. 
