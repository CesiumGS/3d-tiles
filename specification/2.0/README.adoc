...
3D Tiles is designed to be consistent with glTF 2.1. As such, many core concepts are inherited directly from the glTF 2.1 specification or defined through glTF extensions.
...
The default coordinate system may be overriden with the `3DTILES_geocentric_crs` extension. For example, a tileset's global coordinate system will often be in a WGS 84 Earth-centered, Earth-fixed (ECEF) reference frame (EPSG 4978), but it doesn't have to be, e.g. a power plant may be defined fully in its local coordinate system for use with a modeling tool without a geospatial context.
...
The concepts listed below are normatively defined within the `3DTILES_tileset` glTF extension. More details can be found in the xref:{url-specification-gltf}README.adoc#tileset[3DTILES_tileset] specification.
