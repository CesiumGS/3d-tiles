...
image::Styling/figures/example.png[]
...
____
*Implementation Note:* CesiumJS uses the http://jsep.from.so/[jsep] JavaScript expression parser library to parse style expressions into an https://en.wikipedia.org/wiki/Abstract_syntax_tree[abstract syntax tree (AST)].
____
 ...
 ** <<reference-boolean-expression,`boolean expression`>>
 ** <<reference-color-expression,`color expression`>>
 ** <<reference-conditions,`conditions`>>
  *** <<reference-condition,`condition`>>
 ** <<reference-expression,`expression`>>
 ** <<reference-meta,`meta`>>
 ** <<reference-number-expression,`number expression`>>
* <<reference-point-cloud-style,`Point Cloud Style`>>
...
| *defines*
| `object`
| A dictionary object of <<reference-expression,`expression`>> strings mapped to a variable name key that may be referenced throughout the style. If an expression references a defined variable, it is replaced with the evaluated result of the corresponding expression.
| No

| *show*
| `boolean`, `string`, `object`
| A <<reference-boolean-expression,`boolean expression`>> or <<reference-conditions,`conditions`>> property which determines if a feature should be shown.
| No, default: `true`

| *color*
| `string`, `object`
| A <<reference-color-expression,`color expression`>> or <<reference-conditions,`conditions`>> property which determines the color blended with the feature's intrinsic color.
| No, default: `color('#FFFFFF')`

| *meta*
| `object`
| A <<reference-meta,`meta`>> object which determines the values of non-visual properties of the feature.
| No
|===
...
3D Tiles style <<reference-expression,`expression`>> that evaluates to a Color. See link:/specification/Styling/README.md#expressions[Expressions].
...
An <<reference-expression,`expression`>> evaluated as the result of a condition being true. An array of two expressions. If the first expression is evaluated and the result is `true`, then the second expression is evaluated and returned as the result of the condition.
...
A series of property names and the <<reference-expression,`expression`>> to evaluate for the value of that property.

Additional properties are allowed.

* *Type of each property*: <<reference-expression,`expression`>>
...
| A <<reference-number-expression,`number expression`>> or <<reference-conditions,`conditions`>> property which determines the size of the points in pixels.
| No, default: `1`
|===
